...
class App {
    constructor(
        ...
        this.autoScrollInfo = {
            isScrolling: true,
            speed: autoScrollSpeed // Use passed speed
        };
    ...
    onTouchDown(e) {
        this.isDown = true;
        this.autoScrollInfo.isScrolling = false; // Stop auto scroll on interaction
    ...
    update() {
        ...
        if (this.medias) {
            ...
            if (this.medias[0]) {
                const width = this.medias[0].width;
                // The negative logic in scroll is tricky, let's look at absolute difference
                // We want the item closest to center (which corresponds to scroll position)
                // scroll.target grows positively if we drag left (moving right visually).
                // Let's rely on scroll.current
                const rawIndex = Math.round(this.scroll.current / width);
                let index = rawIndex % this.medias.length;
                if (index < 0) index += this.medias.length;

                // mediasImages is 2x the items. 
                // We want origin items index.
...
export default function CircularGallery({
    ...
    useEffect(() => {
        const handleIndexChange = (index) => {
            if (onHoverItem && items && items[index]) {
                onHoverItem(items[index]);
            }
        };
    ...
    }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, onHoverItem]);

    return <div className="w-full h-full overflow-hidden cursor-grab active:cursor-grabbing relative" ref={containerRef} />;
}
